...
# -----------------------------
PREMIUM_FILE = os.path.join(USER_DATA_DIR, "premium_users.json")
...
def load_premium_users():
    try:
        ...
        if logger:
            logger.error(f"load_premium_users error: {e}")
    return {}
...
def save_premium_users(data: dict):
    try:
        ...
        if logger:
            logger.error(f"save_premium_users error: {e}")


def is_premium_user(user_key: str) -> bool:
    users = load_premium_users()
    return bool(users.get(user_key, {}).get('premium', False))


def _ensure_daily_fields(u: dict):
    # ensure daily tracking fields exist and reset if date changed
    today = datetime.utcnow().date().isoformat()
    if u.get('dailyDate') != today:
        u['dailyDate'] = today
        u['dailyMessagesSent'] = 0


def increment_daily_count(user_key: str, amount: int = 1) -> int:
    users = load_premium_users()
    u = users.get(user_key, {})
    if u.get('premium'):
        users[user_key] = u
        save_premium_users(users)
        return u.get('dailyMessagesSent', 0)
    _ensure_daily_fields(u)
    u['dailyMessagesSent'] = u.get('dailyMessagesSent', 0) + amount
    users[user_key] = u
    save_premium_users(users)
    return u['dailyMessagesSent']


def get_daily_count(user_key: str) -> int:
    users = load_premium_users()
    u = users.get(user_key, {})
    _ensure_daily_fields(u)
    return int(u.get('dailyMessagesSent', 0))


def is_over_daily_limit(user_key: str, limit: int = 15) -> bool:
    if is_premium_user(user_key):
        return False
    users = load_premium_users()
    u = users.get(user_key, {})
    _ensure_daily_fields(u)
    return int(u.get('dailyMessagesSent', 0)) >= limit


@app.post('/paddle/webhook')
async def paddle_webhook(request: Request):
    """Simple Paddle webhook receiver.

    Expects Paddle to send form-encoded fields. Uses `passthrough` field
    to identify the user (recommended). Marks that user as premium in
    a local JSON file. This is intentionally simple for demo/testing.
    """
    try:
    ...
    alert = data.get('alert_name') or data.get('alert', '')
    passthrough = data.get('passthrough') or 'local_default_user'
    user_key = passthrough

    # Verify Paddle signature if possible
    p_signature = data.get('p_signature')
    # Support getting public key from settings or environment or file
    public_key = None
    try:
        current_settings = await load_settings()
        public_key = (current_settings.get('systemSettings') or {}).get('paddle_public_key')
    except Exception:
        public_key = None
    if not public_key:
        public_key = os.environ.get('PADDLE_PUBLIC_KEY')
    # also support file USER_DATA_DIR/paddle_public_key.pem
    if not public_key:
        try:
            pk_file = os.path.join(USER_DATA_DIR, 'paddle_public_key.pem')
            if os.path.exists(pk_file):
                public_key = open(pk_file, 'r', encoding='utf-8').read()
        except Exception:
            public_key = None

    def php_serialize(obj):
        # Minimal PHP serializer adequate for typical Paddle webhook payloads
        if obj is None:
            return 'N;'
        ...
        if isinstance(obj, str):
            b = obj.encode('utf-8')
            return f's:{len(b)}:"{obj}";'
        if isinstance(obj, (list, tuple)):
            parts = []
            for i, v in enumerate(obj):
                parts.append(php_serialize(i))
                parts.append(php_serialize(v))
            return f'a:{len(obj)}:{{' + ''.join(parts) + '}'
        if isinstance(obj, dict):
            parts = []
            # Paddle expects keys sorted
            for k in sorted(obj.keys()):
                parts.append(php_serialize(str(k)))
                parts.append(php_serialize(obj[k]))
            return f'a:{len(obj)}:{{' + ''.join(parts) + '}'
        # fallback to string
        return php_serialize(str(obj))

    def verify_paddle_signature(form_data: dict, signature_b64: str, public_key_pem: str) -> bool:
        try:
            if not signature_b64 or not public_key_pem:
                return False
            # lazy import of crypto
            try:
                ...
                try:
                    ...
                    if logger:
                        logger.warning('Crypto library not available for Paddle signature verification')
                    return False

            sig = base64.b64decode(signature_b64)
            # Remove p_signature from data and build serialized payload
            payload = {k: v for k, v in form_data.items() if k != 'p_signature'}

            # Convert values: if single-item lists -> value, else keep list
            normalized = {}
            ...
            serialized = php_serialize(normalized)
    ...
    # If p_signature and a public key are present, verify the signature
    if p_signature and public_key:
        try:
            ok = verify_paddle_signature(data, p_signature, public_key)
            if not ok:
                if logger:
                    logger.warning('Paddle webhook signature verification failed')
                return JSONResponse({'status': 'signature_invalid'}, status_code=400)
        except Exception as e:
            if logger:
                logger.error(f'Error during paddle signature verify: {e}')
            return JSONResponse({'status': 'signature_error'}, status_code=500)

    # treat several alert names as successful purchase/subscription
    success_signals = [
    ...
    if any(sig in (alert or '') for sig in success_signals):
        users = load_premium_users()
        users[user_key] = {
            'premium': True,
            'since': time.time(),
...
async def get_user_premium(passthrough: str = 'local_default_user'):
    users = load_premium_users()
    u = users.get(passthrough, {})
    return JSONResponse({
        'premium': bool(u.get('premium', False)),
        'since': u.get('since'),
        'dailyMessagesSent': u.get('dailyMessagesSent', 0)
    })
...
async def chat_endpoint(request: ChatRequest,fastapi_request: Request):
    ...
    # identify user from passthrough (client should include local sap_user_id)
    user_key = request.passthrough or str(fastapi_request.query_params.get('passthrough') or 'local_default_user')
    ...
    if model == 'super-model':
        current_settings = await load_settings()
        # prepare settings copy to enforce feature gating for non-premium users
        is_premium = is_premium_user(user_key)
        settings_for_call = copy.deepcopy(current_settings)
        if not is_premium:
            # disable server-side features for free users
            if 'ttsSettings' in settings_for_call:
                settings_for_call['ttsSettings'] = settings_for_call.get('ttsSettings', {}).copy()
                settings_for_call['ttsSettings']['enabled'] = False
            if 'text2imgSettings' in settings_for_call:
                settings_for_call['text2imgSettings'] = settings_for_call.get('text2imgSettings', {}).copy()
                settings_for_call['text2imgSettings']['enabled'] = False
        ...
        try:
            # enforce daily free-tier limit
            if is_over_daily_limit(user_key, limit=15):
                return JSONResponse(status_code=403, content={"error": {"message": "Free daily message limit reached. Upgrade to premium.", "type": "rate_limited"}})
            # increment usage (counts per user per day)
            increment_daily_count(user_key, 1)

            if request.stream:
                return await generate_stream_response(client,reasoner_client, request, settings_for_call,fastapi_base_url,enable_thinking,enable_deep_research,enable_web_search,async_tools_id)
            return await generate_complete_response(client,reasoner_client, request, settings_for_call,fastapi_base_url,enable_thinking,enable_deep_research,enable_web_search)
        except asyncio.CancelledError:
            # 处理客户端中断连接的情况
        ...
        try:
            # For agent-specific settings, also apply premium gating
            is_premium = is_premium_user(user_key)
            settings_for_call = copy.deepcopy(agent_settings)
            if not is_premium:
                if 'ttsSettings' in settings_for_call:
                    settings_for_call['ttsSettings'] = settings_for_call.get('ttsSettings', {}).copy()
                    settings_for_call['ttsSettings']['enabled'] = False
            ...
            if request.stream:
                return await generate_stream_response(agent_client,agent_reasoner_client, request, settings_for_call,fastapi_base_url,enable_thinking,enable_deep_research,enable_web_search,async_tools_id)
            return await generate_complete_response(agent_client,agent_reasoner_client, request, settings_for_call,fastapi_base_url,enable_thinking,enable_deep_research,enable_web_search)
        except asyncio.CancelledError:
            # 处理客户端中断连接的情况
            print("Client disconnected")
            raise
...
async def simple_chat_endpoint(request: ChatRequest):
    ...
    current_settings = await load_settings()
    # identify user and enforce free-tier limits
    user_key = request.passthrough or 'local_default_user'
    if is_over_daily_limit(user_key, limit=15):
        return JSONResponse(status_code=403, content={"error": {"message": "Free daily message limit reached. Upgrade to premium.", "type": "rate_limited"}})
    increment_daily_count(user_key, 1)
    # apply premium gating to local settings copy
    is_premium_flag = is_premium_user(user_key)
    settings_for_call = copy.deepcopy(current_settings)
